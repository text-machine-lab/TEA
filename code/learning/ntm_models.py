...
def get_ntm_model2(batch_size=100, m_depth=256, n_slots=100, ntm_output_dim=128, shift_range=3, max_len=15,
    ...
    hidden_lstm = TimeDistributed(Dense(128, activation='sigmoid'))(concat)
    hidden_lstm = Dropout(0.5)(hidden_lstm)

    left_encoder = TimeDistributed(Dense(64, activation='sigmoid'))(left_branch)
    right_encoder = TimeDistributed(Dense(64, activation='sigmoid'))(right_branch)

    encoder = concatenate([left_encoder, hidden_lstm, right_encoder])  # (batch, 256)
    ...
    ntm_layer = ntm(encoder)
    ntm_layer = Dropout(0.5)(ntm_layer)
    hidden_ntm = TimeDistributed(Dense(64, activation='sigmoid'))(ntm_layer)
